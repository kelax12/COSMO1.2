-- ═══════════════════════════════════════════════════════════════════
-- 161 · Cycle de vie des membres (audit 2026-09-23, M10 + M11)
-- ═══════════════════════════════════════════════════════════════════
--
...
-- ─── 1bis · Les lectures d'équipe exigent un membre ACTIF ───────────
--
-- `my_team_project_ids` (mig. 113) lit `organization_members` en direct, sans
-- passer par `is_org_member` : un membre suspendu aurait continué de lire
-- projets et tâches par `get_my_team_projects` / `get_my_team_tasks`. Reprise
-- à l'identique de la 113, avec la seule condition d'activité en plus.
--
-- ⚠️ À GARDER dans toute redéfinition ultérieure (M2 la redéfinit) : la ligne
-- `AND public.is_org_member(p_org)` de `my_membership`.
...
-- ─── 2 · Retrait : reposé tel quel (mig. 106) ───────────────────────
--
-- Reposé ici parce que `CREATE OR REPLACE` sur `is_org_member` ne le touche
-- pas, mais que l'assistant de départ l'appelle : on le fige dans sa forme
-- connue. Quand les membres directs de projet (M2) arriveront, leur purge
-- s'ajoutera ICI, comme celle des équipes à la mig. 104.
...
-- KR sans responsable), sauf pour les subordonnés,
...
BEGIN
  ...
  -- Projets portés : `p_projects_to` est déjà dans la signature pour que le
  -- front n'ait pas à changer quand le responsable de projet (M2) arrivera.
  -- Il n'y a aujourd'hui rien à transférer : `v_projects` reste à 0.
...
AS $$
  SELECT CASE WHEN NOT public.is_org_admin(p_org) THEN NULL ELSE jsonb_build_object(
    ...
    -- Responsable de projet : arrive avec M2.
    'projects', 0,
...
--
-- `team_key_results.contributor_ids` (mig. 160) est un UUID[] sans clé
