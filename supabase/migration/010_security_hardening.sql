...
-- - Bloque la mutation de user_id (mass-assignment) sur tasks/habits/okrs/events/categories/lists
...
-- ─── 2. Tasks : restreindre l'exposition des pending_invites ─────────
-- La policy d'origine permet aux collaborateurs de lire la ligne entière, y
-- compris les emails d'autres invités. On garde la SELECT collaborator mais on
-- attend que la lecture côté client filtre les colonnes sensibles. Pour
-- empêcher un collaborateur de SELECT pending_invites via le client, on déplace
-- la liste des invitations vers une vue restreinte n'incluant que le owner.
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      WHERE fr.status = 'accepted'
        AND (
          (fr.user_id = auth.uid() AND fr.email = friends.email)
          OR (fr.email = (SELECT email FROM auth.users WHERE id = auth.uid())
              AND fr.user_id = (SELECT id FROM auth.users WHERE email = friends.email))
        )
    )
  );

-- ─── 4. friend_requests : SELECT côté destinataire + UPDATE limité ───
-- (Idempotent : on recrée si la migration drift a déjà ajouté receiver_id)
