-- ═══════════════════════════════════════════════════════════════════
-- 162 · Notifications, suivi, journal d'audit, revues (audit 2026-09-23)
-- ═══════════════════════════════════════════════════════════════════
--
...
-- lisible par les admins seuls, purgé à un an.
--
...
-- `project_at_risk` : le type est déclaré ci-dessus, son déclencheur viendra
-- avec la santé déclarée d'un projet (M2, autre session). Il n'écrit rien
-- tant que la colonne `health` n'existe pas, et n'est donc pas posé ici.
...
-- Suppression de tâche : la corbeille (M4, mig. 152, autre session) passe
-- par des RPC. Le journal y sera branché quand elle sera appliquée, pas ici :
-- un trigger sur une colonne qui n'existe pas encore ferait échouer la 162.
...
--   SELECT tgname FROM pg_trigger WHERE tgname LIKE 'trg_audit_%';     -- 6
-- ═══════════════════════════════════════════════════════════════════
