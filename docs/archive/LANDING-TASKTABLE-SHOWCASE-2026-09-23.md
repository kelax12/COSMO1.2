# ⚠️ ARCHIVE — vitrine « tableau des tâches » de la landing, version du 2026-09-22

> ⚠️ **Instantané daté, NON MAINTENU.** Ce fichier conserve
> `src/components/showcase/TaskTableShowcase.tsx` **tel qu'il était avant sa
> refonte du 2026-09-23**. Il n'est chargé par rien : ni build, ni ESLint, ni
> `tsc`, ni les gardes. **Le code fait foi contre cette archive.**

Rejouable depuis l'historique : `git show b683aa16:src/components/showcase/TaskTableShowcase.tsx`.

## Pourquoi elle a été remplacée

- **Elle était entièrement statique** : aucune animation, alors que les quatre
  autres vitrines de la section jouent une boucle.
- **Elle ne ressemblait plus au produit.** Le tableau desktop réel a perdu sa
  colonne « Créé », gagné une colonne « Catégorie », et remplacé ses cinq
  icônes d'action par un menu « … » unique.
- **Son échelle de priorités était inversée** : la vitrine peignait la 5 en
  rouge, alors que dans l'app la **1** est la plus urgente (`task-priority-1`,
  rouge) et la 5 la plus basse (gris).
                ...
                <td className="px-3 py-3.5">
                  <div className="flex items-center gap-2">
                    ...
                    {task.isCollaborative && (
                      <span className="text-[10px] bg-[rgb(var(--color-accent-solid))] text-[rgb(var(--color-accent-solid-foreground))] px-2 py-0.5 rounded-full font-semibold shrink-0">
                        Collaboratif
                      </span>
                    )}
                  </div>
