...
| F-1 | 🟠 Medium | Agenda manager (mig. 077) : `events_manager_*` donne aux admins/managers un accès **lecture+écriture+suppression sur TOUT l'agenda personnel** des membres gérés, sans distinction perso/pro | ✅ Corrigé | `080_security_audit_fable_2026_07_18.sql` (UPDATE/DELETE manager supprimées) + `081_event_privacy_owner_transfer.sql` (colonne `events.is_private`, policies manager `AND NOT is_private`) + `EventModal.tsx` (2026-07-20) : le choix « Privé » est retiré du modal — `isPrivate` forcé côté client selon le contexte (espace personnel → toujours privé ; `MemberAgendaSheet` côté manager → toujours public), arbitrage produit tranché. Voir [F-1](#f-1--agenda-manager--porte-trop-large-sur-lagenda-personnel) |
| F-2 | 🟠 Medium | RGPD : `delete-account` ne purge pas `team_tasks.assignee_ids` (UUID[], pas de FK) → l'UUID d'un compte supprimé survit dans les tâches d'équipe | ✅ Corrigé | `080_security_audit_fable_2026_07_18.sql` (RPC `purge_user_from_team_assignments`, service_role only) + `delete-account/index.ts` (appel avant le drop auth). Appliqué + vérifié live (fonction existe, GRANT service_role uniquement). Voir [F-2](#f-2--rgpd--assignee_ids-non-purgé-à-la-suppression-de-compte) |
...
Rapport détaillé : `AUDIT-SECURITE-2026-07-26.md` (supprimé — contenu repris ici).
...
Voir [`docs/PERFORMANCE.md`](../PERFORMANCE.md) : GSAP supprimé (P-1), Recharts lazy
