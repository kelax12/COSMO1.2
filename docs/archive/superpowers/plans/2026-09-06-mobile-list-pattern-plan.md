> ⚠️ **ARCHIVE — plan ABANDONNÉ, instantané du 2026-09-06, non maintenu.**
>
> **Archivé le 2026-09-21, sur décision d'Axel**, après que la relecture du même jour a établi
> qu'il n'avait **jamais été exécuté** et que sa prémisse ne tenait plus. Ce n'est donc pas un plan
> « exécuté » comme les autres de ce dossier : c'est un plan **abandonné**, et il est rangé ici
> avec sa raison plutôt que laissé à faire croire qu'il reste du travail en cours.
>
> **Ce qui a été mesuré dans le code le 2026-09-21 :**
>
> - `ListRow` et `FilterChips` **n'existent pas** dans `src/components/mobile/`. Le baril
>   `index.ts` porte la trace inverse : « `MobileScreen` et `ListRow` ont été SUPPRIMÉS le
>   2026-09-05 (C-10) ». Ce plan a été écrit **le lendemain de leur suppression**, et proposait
>   d'en recréer une.
> - Le seul commit qui le cite est `3bd8f864`, qui **ajoute ce fichier** et rien d'autre. Aucune
>   de ses 52 cases n'a jamais été cochée.
> - Le correctif d'accès aux Habitudes qu'il embarquait, lui, **a été livré** — séparément, par
>   `80916684`.
>
> 🔴 **Sa prémisse est morte entre-temps.** L'interface mobile a été refondue du 2026-09-16 au
> 09-20 (`ab72cd39` et une trentaine de commits derrière) : tableau de bord, tâches, agenda, OKR,
> catégories, habitudes, statistiques, recherche. L'exécuter tel quel poserait un pattern sur des
> écrans qui n'existent plus sous cette forme.
>
> ⚠️ **Ce que cette archive n'éteint pas** : le besoin d'origine — des listes mobiles qui
> n'empilent pas une « card » par élément — n'a jamais été tranché sur le fond. Il a été traité
> ailleurs et autrement (chips de filtre `0d85a388`, puis la refonte). Si quelqu'un veut le
> reprendre, c'est un plan **neuf** contre les écrans d'aujourd'hui, pas celui-ci.
>
> ❌ **Ne pas le coller à une session.** Le code fait foi contre une archive.
