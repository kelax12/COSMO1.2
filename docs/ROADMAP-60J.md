...
| 🟡 T-06 | Sécurité | (a) ✅ **2FA posée sur le compte Supabase d'Axel le 2026-08-29**. (b) 🔴 **La mig. `131` a été appliquée en prod le 2026-08-31, AVANT tout enrôlement** — l'ordre inverse de celui prescrit. Et la porte de sortie prévue était cassée : l'écran d'enrôlement levait une exception **en phase de rendu**, donc l'`AppErrorBoundary` affichait « Une erreur inattendue s'est produite » au lieu du QR code. **`/admin` est donc resté verrouillé du 2026-08-31 au 2026-09-01** (mesuré : `auth.mfa_factors` = 0 ligne). ✅ **Corrigé et déployé en prod le 2026-09-01** (`formatSecret` rendue totale, réponse d'enrôlement validée à la frontière, `svgToDataUri` durci), vérifié dans le bundle servi. **Reste, et c'est le seul geste : ouvrir `/admin`, enrôler un authentificateur, vérifier que les chiffres reviennent.** | `/admin` expose toute la volumétrie business et n'est protégé que par un mot de passe | P0 | (a) XS · (b) M | — | (a) X · (b) A + B | S1 |
