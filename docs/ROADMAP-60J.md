...
| ✅ T-02 | Fiabilité | ~~Reformulé le 2026-08-29~~ **fait le 2026-09-01** : drill exécuté avec succès dans `.github/workflows/restore-drill.yml` (2 correctifs de script en route : SIGPIPE sur `pg_restore --list \| head`, `bash -e` du runner qui tuait l'étape de restauration avant de compter les erreurs attendues, et un bug de contrôle d'isolation qui capturait le mot `ROLLBACK` au lieu du chiffre réel — les trois corrigés et re-testés). Résultat mesuré : RTO **163 s**, 0 table sans RLS, 129 policies, isolation vérifiée (289/724 tâches visibles par un seul utilisateur). Détail dans `DEPLOYMENT.md` §7. Test humain (login + création manuelle) **volontairement sauté** : l'app a chargé sans erreur sur le projet restauré, l'isolation était déjà prouvée par le contrôle automatisé. Projet jetable et secret `DRILL_DB_URL` supprimés | Un backup non testé n'est pas un backup, et il l'est d'autant moins qu'il est désormais le seul | **P0** | M | ~~T-01~~ T-46 | X + A | S2 |
...
| 🟡 T-23 | UX / Produit | ✅ **Correction livrée le 2026-09-02** : le premier écran d'un compte réel et vide est `src/components/onboarding/FirstRunSetup.tsx`, trois questions passables (tâches, habitude, objectif) dont les réponses deviennent de vraies données. Chaque étape crée **au moment où elle est validée**, jamais à la fin : quelqu'un qui répond à la première question puis ferme l'onglet garde sa tâche, et c'est exactement la population qu'on essaie de retenir. Monté dans `Layout` et non sur une route, parce qu'une inscription par Google ne repasse pas par `SignupPage`. Les **3 tâches d'exemple sont supprimées** : elles étaient écrites en dur en français, hors catalogues i18n, et faisaient agir le produit à la place de la personne. 25 tests. **Reste** : la recette (une action utile dans les 60 premières secondes) et la remesure de l'activation deux semaines après. ⚠️ **L'instrumentation EXISTAIT DÉJÀ** (`get_admin_stats` v3 → `/admin` : activation 24 h, activation 48 h par canal, rétention J+7 par cohorte). Reste la correction seule. Instrumenter et corriger l'activation : **0 compte actif sur 7 jours** pour 28 comptes. Identifier le décrochage (première tâche créée ? deuxième session ?) et traiter le premier écran après inscription | Le vrai problème produit du dossier. Acquérir des utilisateurs qui ne reviennent pas est un coût, pas un progrès | P1 | L | T-15 | B | S3-S4 |
...
- [x] **T-02** — drill de restauration **du dump quotidien** vers un projet jetable · P0 · M · X + A · dép. **T-46** · cf. `DEPLOYMENT.md` §7
  **Done** : RTO 163 s, 0 table sans RLS, 129 policies, isolation vérifiée (289/724). Test humain
  sauté (décision assumée, cf. §7). Projet jetable et secret `DRILL_DB_URL` supprimés le 2026-09-01.
...
- [x] **T-15** — chaîne `?ref=` validée de bout en bout · P0 · S · X
  **Done** : compte jetable créé via `?ref=test_manuel`, `acquisition_source = 'test_manuel'`
  vérifié en base le 2026-08-31, compte supprimé.
...
- [~] **T-23 (correction)** — traiter le premier écran après inscription · P1 · L · B
  ...
  **Code livré le 2026-09-02** (`FirstRunSetup`, 25 tests) ; les deux moitiés du critère « Done »
  restent ouvertes, et aucune ne se prouve depuis le dépôt : la recette demande un compte neuf, la
  remesure demande deux semaines de comptes réels.
...
> ✅ **Les cinq jobs sont VERTS sur `main` depuis le 2026-09-01** (run `33516178297`). Le job
> `e2e` était rouge depuis le 2026-08-30, et **le produit n'y était pour rien** : les tests de
> dépendances cherchaient `getByRole('menuitem')`, hérité d'une version où le sélecteur était un
> menu déroulant, devenu depuis un dialogue à cases à cocher. Une requête sur un rôle qui
> n'existe plus ne trouve rien, et ne dit pas qu'elle cherche au mauvais endroit. Troisième fois
> dans ce dépôt qu'un rouge de CI vient du test et non du code : **avant de suspecter le
> produit, vérifier que le test décrit encore l'interface**.
