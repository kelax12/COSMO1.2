...
| # | À faire | Pourquoi c'est toi | Statut tenu dans |
|---|---|---|---|
| **M-57** 🔴 | **Appliquer la migration `150`** (`150_support_reports_counter.sql`). ⚠️ **Relu en base le 2026-09-21 : elle n'est PAS au ledger**, dont la dernière entrée reste `20260920105522` (mig. `149`) | Tant qu'elle dort, `/admin` affiche « non installé » : la console annonce elle-même qu'elle ne sait pas compter. ⚠️ Relire le ledger **avant** d'appliquer, ce dépôt a déjà appliqué deux fois la même migration | `a-faire-code.md` C-110 |
| **M-58** 🔴 | **Poser la référence des réglages d'auth Supabase** (`npm run check:supabase-posture -- --update`), **puis la commiter** | 🔴 **La garde échoue tant que ce n'est pas fait, et c'est voulu** : un secret ou une référence absents se soldent par un échec, jamais par un avertissement dans un run vert. C'est la règle posée le 2026-09-03 après `uptime.yml`. Le volet manuel qu'elle outille reste `M-15` / `M-16` / `M-17` | `a-faire-code.md` C-88 · `docs/SECURITY.md` |
