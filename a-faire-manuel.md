...
> 🔴 **Les trois secrets que le § 11.1 de `a-faire-code.md` appelle sont, dans ce tableau, M-34 (`RATE_LIMIT_SALT`, côté Supabase), M-11 (`OPS_ALERT_WEBHOOK_URL`, secrets **Actions**) et M-12 (`CRON_SECRET`, Supabase **et** Actions, même valeur des deux côtés).** Ce sont les seuls endroits où du code écrit, testé et committé ne produit **rien** en production.
>
> Un seul des trois a un ordre contraignant : **M-34 se pose avant** que `report-bug` soit redéployée, sinon le signalement de bug répond en erreur entre les deux gestes. ✅ **Remesuré le 2026-09-12 : la mig. `139` est appliquée**, `consume_rate_limit` et `rate_limits` existent en base, la borne a été jouée (4ᵉ appel refusé, 5ᵉ aussi, compteur stabilisé à `limite + 1`). **M-34 est désormais le SEUL geste qui manque avant le redéploiement** — et tant qu'il manque, la RPC est en base et personne ne l'appelle : mesuré le même jour, 12 appels d'affilée à `report-bug` (v8, 2026-08-29) rendent 12 × `400`, aucun refus.
...
| M-41 | 🔴 **Trancher la migration React 19, qui est FAITE et BLOQUÉE** | ⚠️ **Ce n'est plus « quand la planifier » : elle a été jouée le 2026-09-12**, branche `feat/react-19` (poussée, **non fusionnable**). Tout est vert — typecheck, lint 0 erreur, 2 500 tests, les six gates, le build, et l'E2E (104 passés, 0 échec) — **sauf `check:bundle`, exit 1**. React 19 pèse **+23,3 ko gzip** et le chemin critique passe à **329,8 ko** pour un plafond de **323,0**. Ce plafond a été ABAISSÉ le 2026-09-11 pour satisfaire le critère de sortie de C-14 (5 % de marge) : le remonter rouvrirait C-14 le jour même. **Trois issues, et il faut en choisir une** : (1) différer — l'option par défaut, l'urgence sécurité étant tombée ; (2) sortir `framer-motion` du chemin critique pour de vrai, c'est-à-dire remplacer `MotionConfig` par une lecture CSS/`matchMedia` de `prefers-reduced-motion`, chantier à part avec sa mesure au navigateur **sous `reduce`** ; (3) accepter de rouvrir C-14. ❌ Ne pas « différer `MotionConfig` » naïvement : c'est un fournisseur de CONTEXTE, les premiers écrans rendraient sans `prefers-reduced-motion` |
| M-42 | **Choisir la teinte de `--color-accent` du thème clair**, ou écrire qu'on garde | Le dernier verrou de la gate `axe-core`. Après C-25 (le bouton principal est passé AA sur les deux thèmes fautifs, le 2026-09-12), il reste **9 nœuds reproductibles** : `#2563eb` sur son propre fond teinté à 10 % (`#e3ebfa`), à **4,31:1** pour 4,5 requis. Les corriger demande de foncer `--color-accent` — mais c'est la couleur des **liens et du focus**, donc un **second** arbitrage d'identité, distinct de celui rendu pour C-25 qui ne portait que sur `--color-accent-solid`. `#1d4ed8` rendrait 5,59 sur ce fond. ⚠️ Demander 2 ou 3 teintes **rendues côte à côte dans le produit** avant de trancher : c'est comme ça que l'option « texte foncé sur bleu vif » a été écartée pour C-25, elle lisait comme un bouton désactivé. Sinon : écrire « on garde, voici pourquoi » dans `docs/ACCESSIBILITY.md`, et la dispense `color-contrast` y renvoie nommément | `a-faire-code.md` C-23 | `a-faire-code.md` C-58, `docs/MIGRATION-REACT19.md` |
...
| M-31 | **Appliquer les migrations `137`, ~~`138`~~ et ~~`139`~~** (les deux dernières sont appliquées, le 2026-09-12), chacune relue d'abord, et **le ledger relu EN BASE avant** — pas seulement après. Vérifier ensuite **acteur par acteur, dans une transaction annulée par un `RAISE` final**, comme les mig. 130 à 135 : ne JAMAIS conclure d'un « success » | Un `git push` n'applique aucune migration. Les trois sont écrites, testées et committées, et **ne produisent rien** tant qu'elles ne sont pas en base : `137` = les identifiants de refus de dépendance (sans elle, le code traduit encore une erreur **par son message anglais**, ce que `CLAUDE.md` interdit) ; `138` = les preuves L215-1 et L221-28 qui survivent à la suppression d'une organisation (sans elle, supprimer une org **détruit** `renewal_notices` et `withdrawal_consents` en cascade) ; `139` = le plafond de débit de `report-bug`. ⚠️ Ce dépôt a déjà appliqué **deux fois** la même migration parce qu'une session voisine était passée avant. 🔴 **Ordre contraignant sur la `139`** : elle vient après M-34 (`RATE_LIMIT_SALT`) et **avant** le redéploiement de `report-bug`, sinon la fonction appelle une RPC absente. Piège de la borne, qui ne se voit qu'en la jouant : le refus doit être `hits > p_limit`, jamais `>=` — sinon le compteur gèle sur la limite et le plafond ne refuse **jamais** | `a-faire-code.md` C-48, C-30, C-31 |
| M-37 | **Déployer `stripe-org-refund`** (`supabase functions deploy stripe-org-refund`), puis jouer un remboursement **réel contre le compte Stripe de TEST** | La fonction **n'existe pas en production** : vérifié le 2026-09-12, 7 Edge Functions actives, elle n'en fait pas partie. Or le remboursement du mois en cours est **promis aux CGU depuis le 2026-09-04**. Le code est écrit, testé (12 cas de calcul, parcours E2E livré le 09-11), et **rien n'a jamais été joué contre Stripe**. ✅ La dépendance croisée est levée : `stripe-webhook` est en v27 depuis le 09-06 avec sa branche `charge.refunded`, donc la ligne compensatoire au journal d'encaissement est en ligne — `stripe-org-refund` peut partir **seule**. Vérifier dans cet ordre : le montant au prorata des mois non consommés, la ligne compensatoire écrite au journal, l'abonnement **relu** par l'écran, et l'absence de second appel. ⚠️ Écrire la version déployée **avec sa date** : un « corrigé » sans date de déploiement décrit un commit, pas la production | `a-faire-code.md` C-65, C-39 |
