...
|---|---|---|---|---|
| ~~M-11~~ ✅ **Déjà POSÉ le 2026-09-02** à 09:13:47 UTC, onze jours avant que ce tableau ne le réclame encore | `OPS_ALERT_WEBHOOK_URL` | Secrets **Actions** du dépôt GitHub | 🔴 **Cette ligne était fausse**, mesurée le 2026-09-13 par `gh secret list` : le secret y est, et `ci-alert.yml` pousse. Exercice à blanc joué le 09-02 (run `33613177985`) **et rejoué le 09-13** (run `34749470125`), **HTTP 204** les deux fois ; **73** alertes réelles poussées depuis, dont les 14 échecs de `Edge deploy drift`. ⚠️ Reste à Axel une seule chose, que rien d'automatique ne peut rendre : **confirmer qu'il voit ces messages dans le salon**. Un 204 dit que l'endpoint a accepté, pas qu'on l'a lu | `a-faire-code.md` C-28 |
...
> 🔴 **Il n'en reste qu'UN depuis le 2026-09-13** : **M-12** (`CRON_SECRET`, Supabase **et** Actions, même valeur des deux côtés). ~~M-34 (`RATE_LIMIT_SALT`)~~ est posé depuis le 09-12, et ~~M-11 (`OPS_ALERT_WEBHOOK_URL`)~~ l'était **depuis le 09-02** sans que ce tableau l'ait jamais vérifié. C'est le seul endroit où du code écrit, testé et committé ne produit **rien** en production.
>
...
| # | À faire | Pourquoi c'est toi | Statut tenu dans |
|---|---|---|---|
| M-30 | **Déployer les fonctions restantes** : `supabase functions deploy delete-account`, puis `renewal-notice` — ~~`report-bug`~~ est déployée depuis le 2026-09-12 (v11), avec `BUG_REPORT_FROM` déjà en place. Vérifier ensuite que le numéro de version a bien augmenté pour les trois | La commande demande les identifiants Supabase du projet. ⚠️ Faire M-29 dans la même séance, sinon le formulaire de bug répond `503` | `a-faire-code.md` C-35 |
