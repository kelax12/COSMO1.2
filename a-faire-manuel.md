...
| M-37 | **Le fond du hero de la landing : cuire les flous, ou assumer une page à 65 ?** | L'audit A-8 a mesuré que les surfaces floutées empilées du premier écran coûtent **91 % du blocage du fil principal** de `/`, contre **0 ms** sur `/guide`. Trois sorties : cuire le fond (image ou `radial-gradient` déjà flou, seule l'opacité reste animée — rendu quasi identique, coût quasi nul), retirer le flou (des halos nets ne rendent pas le même effet), ou assumer et écrire pourquoi. ❌ Inutile d'espérer un réglage technique : `will-change`, `translateZ(0)`, `contain: paint` et un rayon divisé par trois ont tous été mesurés, aucun ne change rien | `a-faire-code.md` C-67 |
...
| M-39 | **Relever l'après sur le runner** : relancer le job `lighthouse` une fois C-67 corrigé, et inscrire **deux passes** de `/` dans l'item | La règle du dépôt tient : une mesure locale ne fait pas foi **entre deux pages**, la charge machine y domine tout. L'avant est déjà pris (56-63, TBT 546 à 1 633 ms, 2026-09-02, deux passes), donc il n'y a plus qu'à refaire le même geste après |
