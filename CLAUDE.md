...
## 📚 Documentation détaillée (charger à la demande selon la zone touchée)
...
- ❌ `navigate('/dashboard')` directement après `loginDemo()` sans `setTimeout`
...
Défini **uniquement** dans `src/modules/auth/AuthContext.tsx` :
...
## Conventions de modèle — champs canoniques (ne jamais réintroduire)
