...
| F-1 | 🟠 Medium | Agenda manager (mig. 077) : `events_manager_*` donne aux admins/managers un accès **lecture+écriture+suppression sur TOUT l'agenda personnel** des membres gérés, sans distinction perso/pro | ✅ Corrigé (partiel) | `080_security_audit_fable_2026_07_18.sql` — `events_manager_update`/`events_manager_delete` supprimées (mortes, aucun chemin client). SELECT+INSERT restent (feature) ; flag `is_private` toujours en reco produit (nécessite UI, hors périmètre 080). Voir [F-1](#f-1--agenda-manager--porte-trop-large-sur-lagenda-personnel) |
