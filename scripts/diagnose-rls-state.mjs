...
/** Une annotation par ligne : c'est la seule sortie lisible sans authentification. */
function annotate(message) {
  const texte = String(message).replace(/\r?\n/g, '%0A');
  if (process.env.GITHUB_ACTIONS) console.log(`::notice title=rls-state::${texte}`);
