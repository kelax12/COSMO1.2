...
const ClaimOrgInvitePage = () => {
  ...
  return (
    ...
    >
      <Link to="/" aria-label={t('claim.backHome')} className="shrink-0">
        <Logo showText />
      </Link>
      <div className="w-full max-w-md bg-[rgb(var(--color-surface))] border border-[rgb(var(--color-border))] rounded-2xl p-8 shadow-2xl">
        ...
        {joinedOrg ? (
          <div className="space-y-4 text-center">
            ...
            >
              {t('claim.discover')} <ArrowRight size={16} aria-hidden="true" />
            </button>
          </div>
          ...
          <div className="space-y-4">
            ...
            >
              {claimMutation.isPending ? t('claim.joining') : t('claim.join')}
            </button>
          </div>
        )}
      </div>
