 ...
 *
 * Quatre chiffres qui décrivent le PRODUIT, pas une clientèle : la landing ne
 * revendique aucun logo client ni aucun volume d'utilisateurs qu'on ne pourrait
 * pas montrer. Un chiffre invérifiable coûte plus cher qu'il ne rapporte.
 */
  ...
  return (
    <div className="border-y border-white/[0.06] bg-white/[0.015]">
      <div className="mx-auto grid max-w-7xl grid-cols-2 divide-x divide-white/[0.06] px-4 sm:px-6 lg:grid-cols-4 lg:px-8">
        {PROOF_METRICS.map(({ value, suffix, labelKey }, index) => (
          <div
            key={labelKey}
            className={`flex flex-col gap-1.5 px-4 py-8 sm:px-6 ${index === 1 ? 'border-r-0 lg:border-r' : ''} ${
