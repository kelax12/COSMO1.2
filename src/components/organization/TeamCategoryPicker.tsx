 ...
 * existante ou en créer une (nom + couleur), même pattern que
 * `OKRCategoryPicker` — mais la valeur remontée est l'ID (relation par FK,
 * pas par nom recopié) : un projet ou une tâche ne doit pas perdre sa
...
const TeamCategoryPicker = ({ orgId, value, onChange }: TeamCategoryPickerProps) => {
  ...
  const [newColor, setNewColor] = useState(TEAM_CATEGORY_COLORS[0]);
    ...
    createCategory.mutate(
      { name, color: newColor },
      {
        onSuccess: (cat) => {
          onChange(cat.id);
          setNewName('');
          setNewColor(TEAM_CATEGORY_COLORS[0]);
          setCreating(false);
        },
      },
    );
      ...
      {creating && (
        <div className="flex flex-wrap items-center gap-2 rounded-lg border border-[rgb(var(--color-border))] p-2">
          ...
          <div className="flex items-center gap-1">
            {TEAM_CATEGORY_COLORS.map((color) => (
              <button
                key={color}
                type="button"
