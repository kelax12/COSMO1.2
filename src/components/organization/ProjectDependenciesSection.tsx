      ...
      <li key={other.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-[rgb(var(--color-hover))]/60">
        ...
        {onRemove && (
          <button
            type="button"
            onClick={onRemove}
            aria-label={t('portfolio.deps.remove', { name: other.name })}
  ...
  return (
    <section aria-labelledby={`deps-${project.id}`} className="rounded-2xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface))] p-4">
      <h3 id={`deps-${project.id}`} className="flex items-center gap-1.5 text-sm font-bold text-[rgb(var(--color-text-primary))] mb-1">
        <Link2 size={14} aria-hidden="true" /> {t('portfolio.deps.title')}
      </h3>
      {waiting > 0 && (
        <p className="text-xs font-semibold text-amber-600 dark:text-amber-400 mb-2">{tp('portfolio.deps.waiting', waiting)}</p>
      )}

      {waitsFor.length === 0 && blocks.length === 0 && (
        <p className="text-xs text-[rgb(var(--color-text-muted))] mb-3">{t('portfolio.deps.none')}</p>
      )}

      {waitsFor.length > 0 && (
        <>
          <p className="text-xs font-semibold uppercase tracking-wide text-[rgb(var(--color-text-muted))] mt-2 mb-1">{t('portfolio.deps.blockedBy')}</p>
      ...
      {blocks.length > 0 && (
        <>
          <p className="text-xs font-semibold uppercase tracking-wide text-[rgb(var(--color-text-muted))] mt-3 mb-1">{t('portfolio.deps.blocks')}</p>
          <ul className="space-y-0.5">
      ...
      {canEdit && candidates.length > 0 && (
        <div className="mt-3">
          <label htmlFor={`deps-add-${project.id}`} className="sr-only">{t('portfolio.deps.add')}</label>
          <select
          ...
          >
            <option value="">{t('portfolio.deps.pick')}</option>
            {candidates.map((p) => (
