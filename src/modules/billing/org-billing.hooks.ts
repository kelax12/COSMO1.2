...
} from './org-billing.repository';
import type { OrgBillingContact, OrgBillingHistoryEntry, OrgSubscription } from './org-billing.types';
...
export const orgBillingKeys = {
  ...
  history: (orgId: string) => [...orgBillingKeys.all, 'history', orgId] as const,
  contact: (orgId: string) => [...orgBillingKeys.all, 'contact', orgId] as const,
};
...
export const useOrgBillingHistory = (orgId: string | undefined, isOwner: boolean) => {
  ...
  return useQuery<OrgBillingHistoryEntry[]>({
    queryKey: orgBillingKeys.history(orgId ?? ''),
...
export const useOrgBillingContact = (orgId: string | undefined, isOwner: boolean) => {
  ...
  return useQuery<OrgBillingContact | null>({
    queryKey: orgBillingKeys.contact(orgId ?? ''),
    queryFn: () =>
...
export const useSaveOrgBillingContact = (orgId: string, userId: string | undefined) => {
  ...
  return useMutation({
    ...
    onSuccess: (contact) => {
      queryClient.setQueryData(orgBillingKeys.contact(orgId), contact);
      toast.success(translator('org').t(contact ? 'billing.contactSaved' : 'billing.contactRemoved'));
    },
    onError: () => {
      toast.error(translator('org').t('billing.contactError'));
    },
  });
};
